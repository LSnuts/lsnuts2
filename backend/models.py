...
class File(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))  # 上传者ID，关联用户表
    upload_time = db.Column(db.DateTime, default=datetime.utcnow)  # 上传时间
    
    user = db.relationship('User', backref=db.backref('files', lazy=True))  # 关联用户对象
...
class Message(db.Model):
    ...
    send_time = db.Column(db.DateTime, default=datetime.utcnow)  # 发送时间
    
    sender = db.relationship('User', foreign_keys=[sender_id], backref=db.backref('sent_messages', lazy=True))  # 发送者关联
    receiver = db.relationship('User', foreign_keys=[receiver_id], backref=db.backref('received_messages', lazy=True))  # 接收者关联
...
class Post(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))  # 发帖者ID
    ...
    user = db.relationship('User', backref=db.backref('posts', lazy=True))  # 发帖者关联
    comments = db.relationship('Comment', backref='post', lazy=True)  # 帖子的评论列表
    likes = db.relationship('PostLike', backref='post', lazy=True)  # 帖子的点赞列表
...
class Comment(db.Model):
    ...
    post_id = db.Column(db.Integer, db.ForeignKey('posts.id'))  # 所属帖子ID
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))  # 评论者ID
    parent_id = db.Column(db.Integer, db.ForeignKey('comments.id'), nullable=True)  # 父评论ID（楼中楼回复）
    create_time = db.Column(db.DateTime, default=datetime.utcnow)  # 评论时间
    
    user = db.relationship('User', backref=db.backref('comments', lazy=True))  # 评论者关联
    replies = db.relationship('Comment', backref=db.backref('parent', remote_side=[id]), lazy=True)  # 子回复列表

# 通知表 - 存储用户收到的评论回复通知
class Notification(db.Model):
    __tablename__ = 'notifications'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))  # 接收通知的用户
    post_id = db.Column(db.Integer, db.ForeignKey('posts.id'))  # 相关帖子ID
    comment_id = db.Column(db.Integer, db.ForeignKey('comments.id'))  # 相关评论ID
